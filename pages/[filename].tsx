...
export default function HomePage(props: AsyncReturnType<typeof getStaticProps>['props']) {
  ...
  //     </Layout>
  //   );
  const title = props?.data?.page?.socialTitle || `About`;
  const description = props?.data?.page?.socialDescription || ``;
  const image = props?.data?.page?.socialImage || `https://gettr.com/media/jason-miller.png`;
  return (
    <>
