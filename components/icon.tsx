...
export const Icon = ({ data, parentColor = '', className = '' }) => {
  ...
  const IconSVG = React.useMemo(() => {
    const iconOptions = theme.icon === 'boxicon' ? biIconOptions : heroIconOptions;
  ...
  const iconColor = data.color ? (data.color === 'primary' ? theme.color : data.color) : theme.color;

  const iconColorClasses =
    iconColorClass[
      parentColor === 'primary' && (iconColor === theme.color || iconColor === 'primary') ? 'white' : iconColor
    ];
  ...
  const Component = React.useMemo(() => {
    ...
    if (data.style == 'circle') {
      ...
      return <IconSVG className={`${iconSizeClass[iconSize]} ${iconColorClasses} ${className}`} />;
    }
