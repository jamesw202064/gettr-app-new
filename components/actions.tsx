...
export const buttonColorClasses = {
  ...
  teal:
    'text-white bg-teal-500 hover:bg-teal-600 bg-gradient-to-r from-teal-400 to-teal-600 hover:from-teal-400 hover:to-teal-500',
  green:
  ...
  red:
    'text-white bg-red-500 hover:bg-red-600 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-400 hover:to-red-500',
  pink:
    'text-white bg-pink-500 hover:bg-pink-600 bg-gradient-to-r from-pink-400 to-pink-600 hover:from-pink-400 hover:to-pink-500',
  purple:
...
export const Actions = ({ parentColor = 'default', className = '', actions }) => {
          ...
          if (action.type === 'button') {
            element = (
              <Link key={index} href={action.link ? action.link : '/'}>
                ...
                >
                  {action.label}
                  {action.icon && <BiRightArrowAlt className={`ml-1 -mr-1 w-6 h-6 opacity-80`} />}
                </button>
              </Link>
            );
          }
              ...
              <Link key={index} href={action.link ? action.link : '/'} passHref>
                <a
                  className={`group inline-flex items-center font-semibold text-lg transition duration-150 ease-out ${
                    parentColor === 'primary' ? `text-white  hover:text-gray-50` : linkButtonColorClasses[theme.color]
                  }`}
                  style={{
                ...
                >
                  {action.label}
                  {action.icon && <BiRightArrowAlt className={`ml-0 mr-0 w-6 h-6 opacity-80`} />}
                </a>
              </Link>
            );
          }
